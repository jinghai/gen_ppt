...
- 输出日志到 `charts/p18/logs/build_original.log`，并在控制台同步打印。
- 直接将微模板目录打包为 `p18-original.pptx`，不进行冗余的 XML 过滤（微模板已是单页）。
...
def assert_micro_template_integrity() -> None:
    ...
    - 必须存在：`slide1.xml`、`presentation.xml`、`presentation.xml.rels`、`[Content_Types].xml`
    - `ppt/embeddings/` 必须存在且仅包含 `.xlsb`
    - `ppt/charts/_rels/` 必须存在（图表关系用于绑定嵌入数据）
    """
...
def _clean_slide1_rels_and_content_types() -> tuple[int, int]:
    """清理 slide1.xml.rels 中的无效关系，以及 [Content_Types].xml 中的无效 Override。"""
    removed_rels = 0
    removed_ct = 0
...
def build() -> None:
    ...
    # 将 TEMPLATE_DIR 下的所有文件以相对路径形式写入 ZIP 根目录
