...
基于 polarity 字段与统一阈值进行三分类（与 p13 一致）：
...
- **数据线配置**:
  ...
  - Neutral线: `#FFC107` (黄色)  
  - Negative线: `#F44336` (红色)
- **数据计算**: 令 `M = max_{d,l} count(l, d)`，则 `Index(lbl, day) = 100 * count(lbl, day) / M`；保留 1 位小数；同一日三线不再相加为 100。
