...
- `fill_from_excel.py`
  ...
  - 渠道识别：渠道图必须在 `chart_mapping` 中显式提供 `channel` 字段（`Forum`、`Online News`、`Blog`、`Instagram`、`YouTube`、`X`）；如未提供则尝试由 `description` 解析，解析失败立即抛错并终止。

## 数据源与过滤
- `metrics-v4-08.db` 表 `brand_metrics_month`：字段 `countryName`、`brand`、`month`、`sov`。
- `neticle-v5-08-etl.sqlite` 表 `mentions_wide`：字段 `countryId`、`keyword_label`、`sourceLabel`、`createdAtUtcMs`。
...
- 渠道映射：来自 `config.yaml` 的 `channel_map`（Forum、Online News、Blog、X、Instagram、YouTube；未匹配归 Other；大小写不敏感的精确匹配）。
...
- `chart_mapping`: `chart1..chart7` 映射到 `main_trend` 与各渠道 `channel_breakdown`
  - 约定：`chart2..chart7` 必须包含显式 `channel` 字段，以配置为权威来源；避免模板编号与视觉标签错位导致数据填充错误。
